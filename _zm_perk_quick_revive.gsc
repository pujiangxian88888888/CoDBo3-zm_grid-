...
function quick_revive_precache()
{
	...
	/***************************** WARDOGSK93: Start *****************************/
	addon_message = "hud|";
	addon_message += PERK_QUICK_REVIVE + "|";
	addon_message += "shader|";
	addon_message += QUICK_REVIVE_SHADER;
...
function quick_revive_register_clientfield()
{
	...
	clientfield::register("toplayer", "vulture_waypoint_revive", VERSION_SHIP, 2, "int");
}
...
function quick_revive_set_clientfield( state )
{
	...
	self clientfield::set_to_player("vulture_waypoint_revive", state);
}
