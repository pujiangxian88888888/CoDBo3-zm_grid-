...
function additional_primary_weapon_precache()
{
	...
	addon_message = "hud|";
	addon_message += PERK_ADDITIONAL_PRIMARY_WEAPON + "|";
	addon_message += "shader|";
	addon_message += ADDITIONAL_PRIMARY_WEAPON_SHADER;
...
function additional_primary_weapon_register_clientfield()
{
	...
	clientfield::register("toplayer", "vulture_waypoint_mule", VERSION_SHIP, 2, "int");
}
...
function additional_primary_weapon_set_clientfield( state )
{
	...
	self clientfield::set_to_player("vulture_waypoint_mule", state);
}
