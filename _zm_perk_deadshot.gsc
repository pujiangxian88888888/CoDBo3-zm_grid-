...
function deadshot_precache()
{
	...
	addon_message = "hud|";
	addon_message += PERK_DEAD_SHOT + "|";
	addon_message += "shader|";
	addon_message += DEADSHOT_SHADER;
...
function deadshot_register_clientfield()
{
	...
	clientfield::register("toplayer", "vulture_waypoint_deadshot", VERSION_SHIP, 2, "int");
}
...
function deadshot_set_clientfield( state )
{
	...
	self clientfield::set_to_player("vulture_waypoint_deadshot", state);
}
